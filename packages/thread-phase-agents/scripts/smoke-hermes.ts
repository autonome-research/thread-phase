/**
 * Real-binary smoke test for `hermesAgent`.
 *
 * Spawns the actual `hermes acp` subprocess (must be in PATH or pass
 * HERMES_BIN), runs initialize → session/new → session/prompt, prints
 * every canonical AgentEvent, and reports the final result.
 *
 * Run:
 *   npx tsx scripts/smoke-hermes.ts [prompt]
 *
 * Requires Hermes to be configured with a working inference endpoint
 * (config.yaml under ~/.hermes/).
 */

import { hermesAgent } from '../src/hermes/index.js';

async function main(): Promise<void> {
  const cwd = process.cwd();
  const prompt = process.argv.slice(2).join(' ') || "Say only the word 'hello' and nothing else.";

  console.log(`[smoke] hermesAgent — cwd=${cwd}`);
  console.log(`[smoke] prompt: ${prompt}\n`);

  const run = hermesAgent.adapter({
    cwd,
    prompt,
    hermesExecutable: process.env.HERMES_BIN ?? 'hermes',
  });
