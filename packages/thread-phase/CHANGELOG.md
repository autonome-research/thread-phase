...
- Live `job:${jobId}` listeners are isolated snapshot observers: synchronous throws and returned-promise rejections cannot alter lifecycle state or results. Added optional `JobRunnerOptions.onLiveEventError` and immutable `LiveEventListenerFailure` diagnostics.
