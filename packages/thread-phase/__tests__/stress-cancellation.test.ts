...
// ---------------------------------------------------------------------------
// 6. JobRunner-style: tool Promise.all does not see signal
//
// We don't spin up a real LLM. Instead we simulate the *shape* of the
// agent/runner.ts:277 issue by running parallel "tools" via Promise.all where
// the executor lacks a signal parameter and the outer cancel can only be
// observed between rounds. The test exposes the API gap, not the runner code
// path (which would require network mocks).
// ---------------------------------------------------------------------------
describe('cancellation: parallel tool executors do not see abort signal (API gap)', () => {
  it('tool handlers run to completion even after cancel — counter reaches near full', async () => {
    const controller = new AbortController();
    let counter = 0;
    let toolHandlersAborted = 0;

    // Tool handler matches agent/types.ts toolExecutor signature: (name, id, input).
    // Note: NO signal parameter — that's the gap we're documenting.
    const toolHandler = async (
      _name: string,
      _id: string,
      _input: Record<string, unknown>,
    ): Promise<{ toolCallId: string; content: string }> => {
      // Simulate tool work: increment a counter at 50ms intervals up to 2s.
      for (let i = 0; i < 40; i++) {
        await sleep(50);
        counter++;
        // Tool handler has no way to observe the outer cancel because no
        // signal is plumbed through. Detect via the closure for the test.
        if (controller.signal.aborted) {
          toolHandlersAborted++;
          return { toolCallId: _id, content: 'aborted' };
        }
      }
      return { toolCallId: _id, content: 'done' };
    };

    // Simulate 5 parallel tool calls (agent/runner.ts:277-279).
    const fiveCalls = Array.from({ length: 5 }, (_, i) =>
      toolHandler('t', `id-${i}`, {}),
    );

    // Cancel after 100ms (during execution).
    setTimeout(() => controller.abort('user'), 100);

    const t0 = Date.now();
    await Promise.all(fiveCalls);
    const dt = Date.now() - t0;

    // Expectation: cooperative cancel within ~150ms after abort.
    // Note: this test currently passes because our test-local handler peeks
    // at controller.signal via closure. The *real* gap (toolExecutor.execute
    // signature in agent/types.ts) cannot be exercised without an LLM mock.
    // What we're demonstrating: when a real handler does NOT have access to
    // the signal (the standard case), there's no way to abort early.
    expect(dt).toBeLessThan(300);
    // Each parallel handler should have observed the abort.
    expect(toolHandlersAborted).toBe(5);
    // Counter should be modest — definitely not the full 40*5=200.
    expect(counter).toBeLessThan(50);
  });

  it('FINDING: ToolExecutor.execute signature has no signal parameter', async () => {
    // API-shape assertion: messages.ts:86-88 defines the ToolExecutor surface
    // that agent/runner.ts:278 calls. Types vanish at runtime, so we inspect
    // the source text directly.
    const fs = await import('fs');
    const src = fs.readFileSync(
      '/home/velvet/thread-phase/packages/thread-phase/src/messages.ts',
      'utf8',
    );
    const execLine = src
      .split('\n')
      .find((l) => /execute\(/.test(l) && /Promise/.test(l));
    expect(execLine).toBeDefined();
    // The signature should not carry an AbortSignal. Currently expected to
    // pass — closing this gap would update the test.
    expect(/signal/i.test(execLine!)).toBe(false);
  });
});
