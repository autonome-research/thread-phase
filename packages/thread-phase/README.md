...
With `heartbeatMs`, `JobRunner` schedules at most one owner-scoped heartbeat at a time. `heartbeatTimeoutMs` bounds how long the runner waits for each attempt and defaults to `heartbeatMs`; custom stores must still bound or cancel their own underlying I/O because the Promise-based `JobStore` contract has no abort parameter. Ownership loss, timeout, or backend failure stops scheduling and fails the run without being misclassified as cancellation. Cancellation that was requested first—through either the caller signal or `runner.cancel()`—remains authoritative; otherwise the first pipeline or heartbeat failure wins. Automatic and manual refreshes use the published boolean-returning `enableHeartbeat(jobId, ownerId)` contract, converting `false` into `JobOwnershipLostError`; legacy custom stores may retain v5.0.0's no-op `heartbeat()` behavior. Unscoped refresh bypasses ownership checks; hosts should reserve `runner.heartbeatAsOperator(jobId)` for explicit recovery operations. The older `runner.heartbeat(jobId)` name remains as a deprecated compatibility alias.
